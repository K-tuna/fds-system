...
1. **왜 트리 스태킹인가?**
   ...
   - 현업에서 증가 중인 트렌드
   - 실시간 추론 가능 (~12ms)
   - **확률 분포 양극화**: 정상 거래 확률 대폭 감소 → 운영 비용 절감
...
4. **실제 달성 성능**
   ...
   > 참고: FDS에서는 F1보다 **AUPRC, Recall**이 더 적합한 지표
...
Q: "왜 트리 스태킹을 사용했나요?"
> "XGBoost, LightGBM, CatBoost 각각의 강점을 결합했습니다. Meta-learner(LogisticRegression)가 각 모델의 예측을 최적 조합하여 AUC 0.92, AUPRC 0.60을 달성했습니다."

Q: "스태킹 적용 후 확률 분포가 어떻게 변했나요?"
> "**확률 분포가 양극화**되었습니다. 정상 거래가 20~30% → 1~2%로 낮아져서, Block 비율은 유지하면서 Hold/Verify 비율이 감소했습니다. 운영 비용 절감 효과가 있습니다."

Q: "스태킹의 단점은?"
> "추론 시간이 단일 모델 대비 2~3배 증가합니다. 하지만 12ms로 실시간 서비스에 충분합니다."
...
1. **왜 Transformer인가?**
   - 2025 벤치마크 논문에서 정형 데이터에 우수한 성능 보고
   - 정형 + 시퀀스 데이터 모두 처리 가능
...
> "2025년 벤치마크 논문에서 정형 데이터에 Transformer가 우수한 성능을 보였습니다. TabTransformer로 정형 데이터에 적용해서 연구 트렌드를 검증하고 있습니다."

Q: "현업에서 Transformer 안 쓰는 이유는?"
> "추론 속도(50-100ms)가 XGBoost(5ms)보다 느리고, SHAP 설명이 제한적입니다. 하지만 HSBC, Featurespace 등에서 도입 중이어서 미래 표준이 될 가능성이 높습니다."
