...
| 단계 | 모델 | AUC | 설명 |
|------|------|-----|------|
| 1 | XGBoost 단독 | 0.92 | 정형 특성 이상치 탐지 |
| 2 | LSTM 베이스라인 | 0.82 | V1~V20 + 시계열 피처 (35개) |
| 3 | LSTM + Optuna 튜닝 | 0.84 | 하이퍼파라미터 최적화 |
| 4 | LSTM + 피처 추가 | 0.86 | XGBoost importance 기반 피처 |
| 5 | **Ensemble (가중 평균)** | **0.89** | 0.6:0.4 앙상블 |
| 6 | **Fusion (논문 기반)** | **0.92+** | XGBoost→LSTM 융합 등 |

**면접 답변:**
> "XGBoost는 단일 거래의 정형 특성만 봅니다. 하지만 사기는 패턴이 있습니다. '소액 → 소액 → 고액' 같은 시퀀스를 잡으려면 시계열 모델이 필요합니다. LSTM으로 최근 거래 시퀀스를 학습해서 XGBoost와 결합했습니다. 단순 앙상블로는 AUC 0.89였지만, 2024년 논문 기반 융합 방식(XGBoost 확률을 LSTM 입력으로)으로 AUC 0.92 이상을 달성했습니다."
...
| LSTM 베이스라인 AUC | >= 0.82 | 테스트셋 평가 (V1~V20 + 시계열 피처) |
| LSTM 튜닝 후 AUC | >= 0.85 | Optuna 튜닝 + 피처 추가 |
| Ensemble AUC | >= 0.89 | 가중 평균 (0.6:0.4) |
| **Fusion AUC** | **>= 0.92** | 논문 기반 융합 (1-8 노트북) |
...
1. **앙상블 전략 (단순 결합)**
   ...
   - 목표 AUC 0.90 미달 시 → Day 8 융합 방식으로 진행
...
3. **성능 비교**
   ...
   ⚠️ **목표 AUC 0.90 미달 시**: Day 8 융합 방식으로 성능 향상
...
Q: "앙상블로 얼마나 성능이 올랐나요?"
> "XGBoost 단독 AUC 0.92, LSTM 튜닝 후 0.85였습니다. Weighted Average (0.6:0.4)로 앙상블하니 0.89로 향상됐지만 목표 0.90에 미달했습니다. 이후 2024년 논문을 조사해서 융합 방식을 적용, AUC 0.92 이상을 달성했습니다."

Q: "왜 두 모델이 상호 보완적인가요?"
> "XGBoost는 단일 거래의 정형 특성(금액 이상치, 시간대 등)을 잡고, LSTM은 거래 시퀀스의 패턴(소액→고액 급증 등)을 잡습니다. 서로 다른 관점에서 사기를 탐지하므로 결합 효과가 큽니다."
...
| 3 | LSTM 피처 선택 (35개+) | Day 4 | "LSTM 피처는 어떻게 선택?" |
| 4 | 앙상블 → 융합 전환 ⭐ | Day 5, 8 | "왜 앙상블 대신 융합?" |
| 5 | 2024 논문 기반 융합 ⭐ | Day 8 | "융합 방법은?" |
| 6 | SHAP 통합 설명 | Day 6 | "앙상블은 어떻게 설명?" |
| 7 | API 응답 시간 최적화 | Day 7 | "응답 시간은?" |

**핵심 메시지:**
> "단순 앙상블로는 목표 성능에 미달해서, 2024년 논문을 조사하고 융합 방식을 적용했습니다. 문제를 인식하고 해결책을 찾아 적용하는 과정을 보여드릴 수 있습니다."
...
| 항목 | 내용 |
|------|------|
| 모델 | XGBoost + LSTM 융합 (2024 논문 기반) |
| 성능 | AUC 0.92+ (단순 앙상블 0.89 → 융합 0.92+) |
| 설명 | SHAP 기반 통합 설명 |
| 배포 | FastAPI + Docker |
| 응답 | < 200ms |
...
2. **"단순 앙상블 한계 → 2024 논문 조사 → 융합 방식"** → 문제 해결 과정
3. **"융합으로 AUC 0.89 → 0.92+ 달성"** → 실측 성능 개선
4. **"SHAP으로 설명 가능한 AI"** → 금융 규제 대응
5. **"Threshold 비용 최적화"** → 비즈니스 관점
