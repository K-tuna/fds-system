...
- `docs/phase2_prd.md` - Phase 2 기획 (MLOps)
- `docs/phase2_study.md` - Phase 2 사전학습 (MLflow, 드리프트, CI/CD)
- `docs/phase2_impl.md` - Phase 2 구현 상세

---

## 📍 현재 진행 상황

**마지막 업데이트**: 2-4 비용 최적화 + pytest 구현 완료
...
| 학습 | 구현 | 주제 | 상태 |
|------|------|------|------|
| 2-S1 | 2-1 | MLflow Tracking | ✅ |
| 2-S2 | 2-2 | Model Registry (Alias) | ✅ |
| 2-S3 | 2-3 | Evidently 드리프트 | ✅ |
| 2-S4 | 2-4 | 비용 함수 + CI/CD | ✅ |
| 2-S5 | 2-5 | A/B 테스트 (Shadow) | ✅ |
| - | 2-6 | Prometheus/Grafana | (구현만) |
...
│   ├── phase1/
│   │   ├── study/          # Phase 1 사전학습 (1-S1 ~ 1-S10)
│   │   └── (구현 노트북)    # 1-1 ~ 1-12
│   └── phase2/
│       ├── study/          # Phase 2 사전학습 (2-S1 ~ 2-S5)
│       └── (구현 노트북)    # 2-1 ~ 2-6
├── src/
...
### 학습-구현 노트북 매칭 규칙 (필수)
- **학습(Study)과 구현(Impl) 노트북 번호는 1:1 매칭 필수**
- 예: 2-S1 → 2-1, 2-S2 → 2-2, ...
- 예외: 2-6 Prometheus/Grafana는 학습 없이 구현만 (PRD 기반)
- 이 규칙을 어기면 안됨
...
14. **Model Registry**: Stage(deprecated) vs Alias(권장) 차이 설명, Champion/Challenger 패턴으로 A/B 테스트 및 롤백
15. **비용 함수**: FN(100만원) vs FP(5만원) 비용 차이 기반 Threshold 최적화, pytest로 모델 테스트 자동화
